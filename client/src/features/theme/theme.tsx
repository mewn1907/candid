    ...
    return (
      <div className="w-full">
        ...
        {open &&
          createPortal(
            <>
              <button className="fixed inset-0 bg-ink-900/30 backdrop-blur-sm z-[60] sm:hidden" onClick={() => setOpen(false)} aria-label="Close theme picker" />
              <div className="fixed bottom-0 inset-x-0 z-[70] sm:hidden bg-paper-100 border-t border-paper-border rounded-t-organic shadow-[0_-12px_30px_rgba(0,0,0,0.12)] p-4 pb-[calc(1rem+env(safe-area-inset-bottom))] animate-slide-up max-h-[75vh] overflow-y-auto" role="dialog" aria-label="Pick wabi style">
                ...
                <div className="flex items-center justify-between mb-3 px-1">
                  ...
                  <button onClick={() => setOpen(false)} className="text-xs font-mono text-ink-500 px-3 py-1.5 rounded-full bg-paper-200 border border-paper-border active:scale-95">Close</button>
                </div>
                  ...
                  {themes.map((t) => {
                    ...
                    return (
                      <button
                        ...
                        role="radio"
                        aria-checked={active}
                        className={`flex flex-col items-center gap-1.5 p-3 rounded-organic-sm transition-all active:scale-[0.98] ${active ? 'bg-paper-200 ring-2 ring-clay shadow-sm scale-[1.02]' : 'bg-white border border-paper-border/60 hover:bg-paper-200/60'}`}
                      >
